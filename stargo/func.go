...
func resultsToGo(funcType reflect.Type, res starlark.Value, err error) ([]reflect.Value, error) {
	...
	if err != nil {
		...
		if len(outs) > 0 && funcType.Out(len(outs)-1) == errorType {
			outs[len(outs)-1] = reflect.ValueOf(err)
			// TODO: do we need to fill in the other outs with zeros?
			return outs, nil
		}
		return nil, err
	}
	...
	default:
		...
		for i := range outs {
			if !iter.Next(&x) {
				return nil, fmt.Errorf("too few results to unpack (got %d, want %d)", i, len(outs))
			}
		...
		if iter.Next(&x) {
			return nil, fmt.Errorf("too many results to unpack (want %d)", len(outs))
		}
	}
	return outs, nil
}

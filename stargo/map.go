...
func mapConvert(iterable starlark.Iterable, mapType reflect.Type) (res reflect.Value, err error) {
	...
	defer iter.Done()
	if mapping, ok := iterable.(starlark.Mapping); ok {
		// iterable mapping (e.g. dict)

		// TODO: starlark: document that a type that
		// implements both iterable and mapping should iterate
		// over the mapping keys.
...
// delete(map, key)
func go٠delete(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	var m goMap
	var k starlark.Value
	if err := starlark.UnpackPositionalArgs(b.Name(), args, kwargs, 2, &m, &k); err != nil {
		return nil, err
	}
	key, err := toGo(k, m.v.Type().Key())
	if err != nil {

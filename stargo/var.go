...
// index and attr ops produce goVar for all addressable things.
// it supports more INDEX/ATTR ops.
// it is followed by ADDRESS or SETFIELD or SETINDEX.
...
//
// TODO: goVar is degenerate. Once you call Index or Attr, you must call Rvalue.
//
...
func (v goVar) Address() starlark.Value { return v.addr() }
func (v goVar) Value() starlark.Value   { return toStarlark(copyVal(v.v)) }
func (v goVar) addr() goPtr             { return goPtr{v.v.Addr()} }
...
func (v goVar) SetField(name string, val starlark.Value) error {
	...
	if v.v.Kind() == reflect.Struct {
		return v.addr().SetField(name, val)
	}
...
func (v goIndexableVar) SetIndex(i int, y starlark.Value) error {
	...
	if v.v.Kind() == reflect.Array {
		return v.addr().SetIndex(i, y) // delegate to address
	}

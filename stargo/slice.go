...
func (s goSlice) SetIndex(i int, v starlark.Value) error {
	...
	if !elem.CanSet() {
		return fmt.Errorf("can't set slice element") // e.g. accessed via unexported field
	}
	elem.Set(x)
	return nil
}
...
// slice(start, end, cap?) -- like Go x[start:end:cap], not Starlark x[start:end:stride].
...
// copy(dest, src)
func go٠copy(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	var dest, src goSlice
	if err := starlark.UnpackPositionalArgs(b.Name(), args, kwargs, 2, &dest, &src); err != nil {
		return nil, err
	}
	return starlark.MakeInt(reflect.Copy(dest.v, src.v)), nil
}
...
// cap(v)
func go٠cap(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	var v starlark.Value
	if err := starlark.UnpackPositionalArgs(b.Name(), args, kwargs, 1, &v); err != nil {
		return nil, err
	}
	switch v := v.(type) {
	case goArray, goSlice, goChan:
		return starlark.MakeInt(v.(Value).Reflect().Cap()), nil
	}
	return nil, fmt.Errorf("cap: got %s, want Go array, slice, or chan", v.Type())
}
